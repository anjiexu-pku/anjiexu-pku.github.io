...
真实传感器通常使用 CFA, 例如 Bayer pattern。每个像素只记录一个颜色通道。令 \(M_c(p)\in\{0,1\}\) 表示像素 \(p\) 是否采样通道 \(c\), 那么 RAW 测量可以写成:
...
$$
\min_{\theta,\psi}
\mathcal{L}_{R}(R_{\theta}(r),s^*)
+
\mathcal{L}_{E}(E_{\psi}(R_{\theta}(r)),y^*).
$$

其中 \(s^*\) 是线性恢复目标, \(y^*\) 是最终增强目标。
...
### 11.3 可逆性决定 RGB-to-RAW 到底是“反变换”还是“估计”
