...
下面只围绕一套符号展开。先写清世界怎样产生像素，再讨论从像素能恢复什么，最后把相关论文放回同一个数学坐标系。
...
Deep Optics 则连测量核也一起学习 [8]。令可制造的光学参数为 $\phi$，成像模拟器为 $\mathcal{M}_{\phi}$，解码器为 $F_{\theta}$:
...
到目前为止，我们一直在讨论一个固定测量之后的反问题: 相机已经拍完了，模型怎样从 $R_a$ 估计 $T$。但如果相机可以改变焦点、曝光、孔径、视角或光照，那么“下一张怎么拍”本身就是推断的一部分。
